...
export const WebRPCSchemaHash = "ec713d7bd2b4bafe4ae496ef51914651a0c3dd83"
...
export interface StoredSignature {
  ...
  signature: string
  type: number
  transaction: StoredSignatureTransaction
}

export interface StoredSignatureTransaction {
  digest: string
  wallet: string
  imageHash: string
  gapNonce: string
...
export interface Sessions {
  ...
  saveWitness(args: SaveWitnessArgs, headers?: object): Promise<SaveWitnessReturn>
  ...
  knownSignaturesOfSigner(args: KnownSignaturesOfSignerArgs, headers?: object): Promise<KnownSignaturesOfSignerReturn>
  knownWitnesses(args: KnownWitnessesArgs, headers?: object): Promise<KnownWitnessesReturn>
  ...
  signaturesForImageHash(args: SignaturesForImageHashArgs, headers?: object): Promise<SignaturesForImageHashReturn>
...
export interface SaveWitnessArgs {
  witness: Witness
  signatures: Array<Signature>
}

export interface SaveWitnessReturn {
  ok: boolean  
}
...
export interface KnownSignaturesReturn {
  signatures: Array<StoredSignature>  
}
export interface KnownSignaturesOfSignerArgs {
  signer: string
  start: number
  count: number
}

export interface KnownSignaturesOfSignerReturn {
  signatures: Array<StoredSignature>  
}
export interface KnownWitnessesArgs {
  start: number
  count: number
}

export interface KnownWitnessesReturn {
...
export interface SignaturesForImageHashArgs {
  imageHash: string
  start: number
  count: number
}

export interface SignaturesForImageHashReturn {
  signatures: Array<StoredSignature>  
}
...
export class Sessions implements Sessions {
  ...
  knownSignatures = (args: KnownSignaturesArgs, headers?: object): Promise<KnownSignaturesReturn> => {
    return this.fetch(
      ...
      return buildResponse(res).then(_data => {
        return {
          signatures: <Array<StoredSignature>>(_data.signatures)
        }
      })
    })
  }
  ...
  knownSignaturesOfSigner = (args: KnownSignaturesOfSignerArgs, headers?: object): Promise<KnownSignaturesOfSignerReturn> => {
    return this.fetch(
      this.url('KnownSignaturesOfSigner'),
      createHTTPRequest(args, headers)).then((res) => {
      return buildResponse(res).then(_data => {
        return {
          signatures: <Array<StoredSignature>>(_data.signatures)
  ...
  knownWitnesses = (args: KnownWitnessesArgs, headers?: object): Promise<KnownWitnessesReturn> => {
    return this.fetch(
      this.url('KnownWitnesses'),
      createHTTPRequest(args, headers)).then((res) => {
      return buildResponse(res).then(_data => {
        return {
          witnesses: <Array<Witness>>(_data.witnesses)
        }
      })
    })
  }
  ...
  signaturesForImageHash = (args: SignaturesForImageHashArgs, headers?: object): Promise<SignaturesForImageHashReturn> => {
    return this.fetch(
      this.url('SignaturesForImageHash'),
      createHTTPRequest(args, headers)).then((res) => {
      return buildResponse(res).then(_data => {
        return {
          signatures: <Array<StoredSignature>>(_data.signatures)
